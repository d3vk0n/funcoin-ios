...
function executeCallback (id, error, value) {
  funcWallet.executeCallback(id, error, value)
}
function onSignSuccessful(id, value) {
  console.log("onSignSuccessful", value)
  funcWallet.executeCallback(id, null, value)
}
function onSignError(id, error) {
  funcWallet.executeCallback(id, error, null)
}
window.funcWallet.init(rpcURL, {
  getAccounts: function (cb) { cb(null, [addressHex]) },
  processTransaction: function (tx, cb){
    console.log('signing a transaction', tx)
    const { id = 8888 } = tx
    funcWallet.addCallback(id, cb)
...
window.web3.setProvider = function () {
  console.debug('funcWallet Wallet - overrode web3.setProvider')
}
...
window.funcWallet = Object()
window.funcWallet.getAccount = function() {
    return addressHex
}
window.funcWallet.getAccounts = function() {
